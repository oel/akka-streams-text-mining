...
object NgramStream {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val countFlow: Flow[String, Int, NotUsed] = Flow[String].map(_ => 1)
    val countSink: Sink[Int, Future[Long]] = Sink.fold[Long, Int](0L)(_ + _)

    val graph = GraphDSL.create(consoleSink, fileSink(filename), countSink)((_, _, count) => count) {
      implicit builder =>
        (console, file, count) =>
    ...
    runGraph.onComplete {
      case Success(x) =>
        println(s"Success: Total text messages processed: $x")
        system.terminate()
      case Failure(e) =>
